 ...
 */
struct AlignedArray {
  AlignedArray(const size_t size) {        // 构造方法,用pybind_class_的pybind:init函数绑定到类名Array上。在python端直接按modles.Array(size)调用,分配连续内存。返回指针
    // 这里按ALIGNMENT(256)字节对齐。
    int ret = posix_memalign((void**)&ptr, ALIGNMENT, size * ELEM_SIZE);  // 返回的ptr指向的内存地址，是256的倍数。共分配size个元素的内存大小
